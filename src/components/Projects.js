...
const Projects = ({ username, selectedRepos }) => {
    ...
    useEffect(() => {
        fetch(`https://api.github.com/users/${username}/repos`)
            ...
            .then(data => {
                console.log('Fetched Repositories:', data); // Log fetched repositories
                const filteredRepos = data.filter(repo => selectedRepos.includes(repo.name));
                console.log('Filtered Repositories:', filteredRepos); // Log filtered repositories
